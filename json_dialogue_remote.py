...
def choose_output_label(
    ...
    system_prompt = (
       "You are a dialogue router for a spoken conversation about setting up a date.\n"
...
def speak(furhat: FurhatRemoteAPI, text: str, gesture: Optional[str] = None) -> None:
    text = text.strip()
    if not text:
        return
    ...
    if resolved_gesture:
        try:
            furhat.gesture(name=resolved_gesture)
...
def listen(furhat: FurhatRemoteAPI) -> str:
    ...
    if isinstance(user_utt, dict):
        user_utt = user_utt.get("message", "")
    elif hasattr(user_utt, "message"):
        user_utt = getattr(user_utt, "message", "")
...
if __name__ == "__main__":
    ...
    furhat = FurhatRemoteAPI(args.host)
    ...
    furhat.set_voice(name="FableTurboMultilingualNeural")
